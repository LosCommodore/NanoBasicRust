...
where I: Iterator<Item = &'a Token> {
    let boolean_expr = BooleanExpression { 
        ...
        left_expr: NumericExpression { node: Node {line_num:2, col_start:3, col_end:4} },
        right_expr: NumericExpression { node: Node {line_num:2, col_start:3, col_end:4} },
    };

    let then_statement: Statement = Statement { line_id:42, node: Node {line_num:2, col_start:3, col_end:4}, kind: StatementEnum::Print};
    let if_statement: IfStatement = IfStatement{boolean_expr, then_statement};
...
where
    I: Iterator<Item = &'a Token>,
 {
    ...
    let TokenType::Number(line_id) =  line_token.kind else { return Err("Parse error".into())};
    ...
    let node = Node {line_num:1, col_start:1, col_end:1};
    let statement = Statement {node, line_id, kind};
    Ok(statement)
}
